...
export default async function ParametresPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-2xl flex-1 px-5 py-10">
      <h1 className="text-2xl font-bold text-texte">Réglages</h1>
      <p className="mt-2 text-sm text-texte-2">
        Connecté avec <span className="text-texte">{user.email}</span>. Toute modification
      ...
      <form action={deconnexion} className="mt-8">
        <button
          type="submit"
          className="rounded-lg border border-filet px-4 py-2.5 text-sm text-texte-2 transition hover:text-texte"
        >
          Se déconnecter
        </button>
      </form>

      <p className="mt-10 text-xs text-texte-3">
